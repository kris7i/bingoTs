...
class WeChat
{
    ...
    {
        $result = \Tools\Http::get(self::API_URL_PREFIX . self::AUTH_URL . 'appid=' . $this->appid . '&secret=' . $this->appsecret);
        if ($result) {
    ...
    private function getWxQRCode($token = '', $url = '', $param = [])
    {
        ...
        $result = \Tools\Http::post($url, json_encode($param));
